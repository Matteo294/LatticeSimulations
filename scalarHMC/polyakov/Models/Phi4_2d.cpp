...
Phi4_2d::Phi4_2d(class Lattice* latt, double m2, double g) : 
    Model(latt),
    phi(latt->Nt, vector<double> (latt->Nx, 0.0)), 
    phicopy(latt->Nt, vector<double> (latt->Nx, 0.0))
    {
    this->m2=m2;
    this->g=g;
...
void Phi4_2d::saveConfiguration(){
    phicopy = phi;
}
void Phi4_2d::writeSavedConfiguration(){
    phi = phicopy;
}
