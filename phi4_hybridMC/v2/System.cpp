...
System::System(class Lattice* l) : phi(l->Nt, vector<double> (l->Nx, 0.0)) {this->lattice = l;};
System::~System(){};
double System::evaluateAction(){
    ...
    for(int nt=0; nt<Nt; nt++){
        for(int nx=0; nx<Nx; nx++){
            s += -0.5 * phi[nt][nx] * (phi[PBCidx(nt+1, Nt)][nx] + phi[PBCidx(nt-1, Nx)][nx] + phi[nt][PBCidx(nx+1, Nx)] + phi[nt][PBCidx(nx-1, Nx)] - 4*phi[nt][nx])
